    ...
    else if (srcType == "adc")
    {
        ...
        if (auto *adcVal = srcVal->Get("adc") ; !adcVal->IsUndefined())
        {
            ...
            if (auto it = adcEntryMap.find(adcStr); it != adcEntryMap.end())
            {
                ...
                int32_t threshold = srcVal->Get("threshold")->Int32(0);
...
Button::ADCSource::ADCSource(ADC* adc, int channel, bool above, int32_t threshold) :
    ...
    // Compare to the threshold and WARN if out of range with min/max
    uint32_t adcRangeMin = adc->GetNativeMin();
    uint32_t adcRangeMax = adc->GetNativeMax();
    if (threshold > adcRangeMax || threshold < adcRangeMin)
    {
        Log(LOG_WARNING, "ADCSource: threshold %i is out of range compared to the ADC min (%i) or max (%i)\n", 
            threshold, adcRangeMin, adcRangeMax);
    }
    Log(LOG_DEBUG, "ADCSource: %s, ADC min (%i), max (%i)\n", fullName, adcRangeMin, adcRangeMax);
}

bool Button::ADCSource::Poll() {
    // Read the adc
    ADC::Sample s = adc->ReadNative(channel);
