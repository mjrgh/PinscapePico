...
// in turn keep the TLC5940 output ports in high-Z state.  This should
...
bool TLC5940::Init(const uint8_t *dcData)
{
    ...
    //
    // The chip can accept SCLK speeds up to 30 MHz.  The Pico CPU
    // clock is 125 MHz, which is 4.167x faster, so we need to pad
    // out the bit-banging loop to at least 5 CPU cycles per loop,
    // or 3 cycles per SCLK up/down op.  It's safe to assume that
    // the loop processing will take at least two cycles.
    ...
    gpio_put(gpXlat, 1);
    __asm volatile ("nop");  // min 20ns pulse width
    __asm volatile ("nop");  // -> 3x 8ns CPU clock
    __asm volatile ("nop");
    gpio_put(gpXlat, 0);
...
void TLC5940::PopulateLevels(PinscapePico::OutputDevLevel* &levels)
{
    for (auto &chain : chains)
    {
        if (chain != nullptr)
        {
            // the level[i] array for each chain is in reverse order of port number
...
void TLC5940::Set(int port, uint16_t newLevel)
{
    ...
    if (port >= 0 && port < nPorts)
    {
        ...
        // shifting left four bits.  (The low-order 4 bits aren't used,
        // so we're not actually rescaling the value - we're merely
        // left-aligning a 12-bit value in a 16-bit container.  The
        // low-order 4 bits of the 16-bit container aren't used.)
