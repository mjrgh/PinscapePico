...
//
// Another optimization is that we don't both much with detailed error
// diagnostics for parsing errors in the JSON source code.  That's not
// to say that we don't handle errors at all: the parser still has to
// recognize ill-formed input and recover gracefully, since it does
// have to work with data generated externally.  The parser can't
// crash or corrupt memory or behavior unpredictably when presented
// with ill-formed input.  But it also doesn't have to provide a lot
// of detailed user-oriented help to explain or pinpoint syntax errors,
// because we expect that the source text will generally be machine-
// generated by the Config Tool.  Any errors in the text thus represent
// bugs in the Config Tool rather than something that an end user is
// expected to fix.  This lets us make the parser code smaller than
// it would be if it had to generate detailed syntax diagnostics,
// since good parsing diagnostics require a lot of added intelligence
// (and thus a lot of added code).
//
...
#define IfValueKeepSourceRef(x)
#else
#define JSON_VALUE_KEEP_SOURCE_REF
#define IfValueKeepSourceRef(x) x
#endif
...
public:
    ...
    struct Value
    {
        ...
        // Object property map.  
        // 
        // e define this as a multimap, to allow redundant property name in an
        // object definition.  A repeated property name is an error, but it's
        // perfectly possible for a user to write JSON code with errors, so it's
        // a case we have to be prepared to handle.  A multimap lets us construct
        // a tree with redundant properties per object without special handling.
        // We'll flag these as errors, but we'll still get a valid parse tree
        // with all properties in the source included in the tree.
        //
        // The key is expressed with our custom counted-length string type, so
        // that we can use a pointer directly into the source text to store the
        // key.  This helps keep the parse tree as compact as possible by
        // eliminating the need to store a separate copy of each property name
        // in the hash table; we just store a pointer and a length.  This
        // requires a custom hash and comparator function for our custom type.
        using PropMap = std::unordered_multimap<
            Value::StringWithLen, PropValue, 
            Value::StringWithLen::HashFunc, Value::StringWithLen::EqualsFunc>;
    ...
    {
        PropValue(const Token &propTok) : propTok(propTok) { }
        PropValue(const Token &propTok, Type type) : Value(type), propTok(propTok) { }
