// Scintilla JSON lexer module - adapted from Lexilla.
// https://www.scintilla.org/Lexilla.html
//
// (The only changes are to make the #include paths explicit.)
...
// added Pinscape style
#define SCE_JSON_NAKEDPROPERTYNAME 14
#define SCE_JSON_HEXNUMBER 15
#define SCE_JSON_LAST_STYLE 15
...
void SCI_METHOD LexerJSON::Lex(Sci_PositionU startPos,
	...
	while (context.More()) {
		...
		case SCE_JSON_ESCAPESEQUENCE:
			...
			break;
		case SCE_JSON_NAKEDPROPERTYNAME:
			if (context.ch == ':') {
				context.SetState(SCE_JSON_DEFAULT);
			}
			break;
		...
		case SCE_JSON_NUMBER:
			if (context.chPrev == '0')
			{
				if (context.ch == 'o' || context.ch == 'O')
					context.SetState(SCE_JSON_NUMBER);
				else if (context.ch == 'x' || context.ch == 'X')
					context.SetState(SCE_JSON_HEXNUMBER);
				else
					context.SetState(SCE_JSON_DEFAULT);
			}
			else
		...
		case SCE_JSON_HEXNUMBER:
			if (IsAHeXDigit(context.ch))
				context.SetState(SCE_JSON_HEXNUMBER);
			else
				context.SetState(SCE_JSON_DEFAULT);
			break;
		...
		if (context.state == SCE_JSON_DEFAULT) {
			...
			else if (iswordstart(context.ch) && context.state != SCE_JSON_KEYWORD) {
				context.SetState(SCE_JSON_NAKEDPROPERTYNAME);
			}
