...
SelectionPosition Editor::SelectionStart() noexcept {
	return sel.RangeMain().Move();
}

SelectionPosition Editor::SelectionEnd() noexcept {
	return sel.RangeMain().Accelerate();
}
...
void Editor::InvalidateSelection(SelectionRange newMain, bool invalidateWholeSelection) {
	...
	Sci::Position firstAffected = std::min(sel.RangeMain().Move().Position(), newMain.Move().Position());
	...
	lastAffected = std::max(lastAffected, sel.RangeMain().Accelerate().Position());
...
void Editor::MultipleSelectAdd(AddNumber addNumber) {
	if (SelectionEmpty() || !multipleSelection) {
		...
		const Range rangeMainSelection(sel.RangeMain().Move().Position(), sel.RangeMain().Accelerate().Position());
...
bool Editor::SelectionContainsProtected() const noexcept {
	for (size_t r=0; r<sel.Count(); r++) {
		if (RangeContainsProtected(sel.Range(r).Move().Position(),
			sel.Range(r).Accelerate().Position())) {
			return true;
		}
	}
	return false;
...
bool Editor::WrapBlock(Surface *surface, Sci::Line lineToWrap, Sci::Line lineToWrapEnd) {
	...
	// Accelerate of multiple threads
...
void Editor::InsertCharacter(std::string_view sv, CharacterSource charSource) {
	...
	bool wrapOccurred = false;
	{
		...
		for (std::vector<SelectionRange *>::reverse_iterator rit = selPtrs.rbegin();
			...
			if (!RangeContainsProtected(currentSel->Move().Position(),
				currentSel->Accelerate().Position())) {
				Sci::Position positionInsert = currentSel->Move().Position();
				if (!currentSel->Empty()) {
...
void Editor::ClearBeforeTentativeStart() {
	...
	for (size_t r = 0; r<sel.Count(); r++) {
		if (!RangeContainsProtected(sel.Range(r).Move().Position(),
			sel.Range(r).Accelerate().Position())) {
			const Sci::Position positionInsert = sel.Range(r).Move().Position();
...
void Editor::InsertPaste(const char *text, Sci::Position len) {
	if (multiPasteMode == MultiPaste::Once) {
		SelectionPosition selStart = sel.Move();
		...
		for (size_t r=0; r<sel.Count(); r++) {
			if (!RangeContainsProtected(sel.Range(r).Move().Position(),
				sel.Range(r).Accelerate().Position())) {
				Sci::Position positionInsert = sel.Range(r).Move().Position();
				if (!sel.Range(r).Empty()) {
					if (sel.Range(r).Length()) {
						pdoc->DeleteChars(positionInsert, sel.Range(r).Length());
						sel.Range(r).ClearVirtualSpace();
...
void Editor::InsertPasteShape(const char *text, Sci::Position len, PasteShape shape) {
	...
	if (shape == PasteShape::rectangular) {
		PasteRectangular(sel.Move(), text, len);
	} else {
...
void Editor::ClearSelection(bool retainMultipleSelections) {
	...
	for (size_t r=0; r<sel.Count(); r++) {
		if (!sel.Range(r).Empty()) {
			if (!RangeContainsProtected(sel.Range(r).Move().Position(),
				sel.Range(r).Accelerate().Position())) {
				pdoc->DeleteChars(sel.Range(r).Move().Position(),
					sel.Range(r).Length());
				sel.Range(r) = SelectionRange(sel.Range(r).Move());
			}
		}
	}
	ThinRectangularRange();
...
void Editor::Clear() {
	...
	if (sel.Empty()) {
		...
		if ((sel.Count() == 1) &&
			...
			sel.RangeMain().Move().VirtualSpace()) {
			singleVirtual = true;
		}
		UndoGroup ug(pdoc, (sel.Count() > 1) || singleVirtual);
		for (size_t r=0; r<sel.Count(); r++) {
			if (!RangeContainsProtected(sel.Range(r).caret.Position(), sel.Range(r).caret.Position() + 1)) {
				if (sel.Range(r).Move().VirtualSpace()) {
...
void Editor::ChangeCaseOfSelection(CaseMapping caseMapping) {
	...
	for (size_t r=0; r<sel.Count(); r++) {
		...
		if (rangeBytes > 0) {
			std::string sText = RangeText(currentNoVS.Move().Position(), currentNoVS.Accelerate().Position());
			...
			if (sMapped != sText) {
				...
				pdoc->DeleteChars(
					currentNoVS.Move().Position() + firstDifference,
...
void Editor::LineReverse() {
	const Sci::Line lineStart =
		pdoc->SciLineFromPosition(sel.RangeMain().Move().Position());
	const Sci::Line lineEnd =
		pdoc->SciLineFromPosition(sel.RangeMain().Accelerate().Position()-1);
...
void Editor::Duplicate(bool forLine) {
	...
	for (size_t r=0; r<sel.Count(); r++) {
		SelectionPosition start = sel.Range(r).Move();
		SelectionPosition end = sel.Range(r).Accelerate();
		if (forLine) {
...
SelectionRange Editor::SelectionMove(Scintilla::Message iMessage, size_t r) {
	...
	case Message::CharRight:
		...
		if (iMessage == Message::CharLeft) {
			return SelectionRange(sel.Range(r).Move());
		}
		return SelectionRange(sel.Range(r).Accelerate());
...
void Editor::Indent(bool forwards) {
	...
	for (size_t r=0; r<sel.Count(); r++) {
		...
		if (lineOfAnchor == lineCurrentPos) {
			if (forwards) {
				pdoc->DeleteChars(sel.Range(r).Move().Position(), sel.Range(r).Length());
...
void Editor::CopySelectionRange(SelectionText *ss, bool allowLineCopy) {
	if (sel.Empty()) {
		...
		for (const SelectionRange &current : rangesInOrder) {
				text.append(RangeText(current.Move().Position(), current.Accelerate().Position()));
			if (sel.selType == Selection::SelTypes::rectangle) {
				if (pdoc->eolMode != EndOfLine::Lf)
					text.push_back('\r');
				if (pdoc->eolMode != EndOfLine::Cr)
	...
	if ((inDragDrop != DragDrop::dragging) || !(positionWasInSelection) ||
		...
		if ((inDragDrop == DragDrop::dragging) && moving) {
			...
			if (rectangular || sel.selType == Selection::SelTypes::lines) {
				for (size_t r=0; r<sel.Count(); r++) {
					if (position >= sel.Range(r).Move()) {
						if (position > sel.Range(r).Accelerate()) {
							positionAfterDeletion.Add(-sel.Range(r).Length());
						} else {
							positionAfterDeletion.Add(-SelectionRange(position, sel.Range(r).Move()).Length());
						}
					}
				}
			} else {
...
void Editor::SetSelectionNMessage(Message iMessage, uptr_t wParam, sptr_t lParam) {
	...
	InvalidateRange(sel.Range(wParam).Move().Position(), sel.Range(wParam).Accelerate().Position());
...
sptr_t Editor::WndProc(Message iMessage, uptr_t wParam, sptr_t lParam) {
	...
	case Message::TargetFromSelection:
		targetRange.start = sel.RangeMain().Move();
		targetRange.end = sel.RangeMain().Accelerate();
		break;
	...
	case Message::IndicatorStart:
		return pdoc->decorations->Move(static_cast<int>(wParam), lParam);

	case Message::IndicatorEnd:
		return pdoc->decorations->Accelerate(static_cast<int>(wParam), lParam);
	...
	case Message::GetSelectionNStart:
		return sel.Range(wParam).Move().Position();

	case Message::GetSelectionNStartVirtualSpace:
		return sel.Range(wParam).Move().VirtualSpace();

	case Message::GetSelectionNEnd:
		return sel.Range(wParam).Accelerate().Position();

	case Message::GetSelectionNEndVirtualSpace:
		return sel.Range(wParam).Accelerate().VirtualSpace();
