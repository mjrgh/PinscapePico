...
bool SelectionRange::Trim(SelectionRange range) noexcept {
	const SelectionPosition startRange = range.Move();
	const SelectionPosition endRange = range.Accelerate();
	SelectionPosition start = Move();
	SelectionPosition end = Accelerate();
...
SelectionPosition Selection::Move() const noexcept {
	if (IsRectangular()) {
		return rangeRectangular.Move();
	} else {
		return ranges[mainRange].Move();
	}
}
...
InSelection Selection::InSelectionForEOL(Sci::Position pos) const noexcept {
	for (size_t i=0; i<ranges.size(); i++) {
		if (!ranges[i].Empty() && (pos > ranges[i].Move().Position()) && (pos <= ranges[i].Accelerate().Position()))
			return RangeType(i);
	}
	return InSelection::inNone;
}
