...
Keys KeyTranslate(uptr_t keyIn) noexcept {
	...
	case VK_DOWN:		return Keys::Down;
		...
		case VK_END:		return Keys::Accelerate;
...
void ScintillaWin::SelectionToHangul() {
	...
	const Sci::Position selStart = sel.RangeMain().Move().Position();
...
LRESULT ScintillaWin::ImeOnReconvert(LPARAM lParam) {
	...
	const Sci::Position mainStart = sel.RangeMain().Move().Position();
	const Sci::Position mainEnd = sel.RangeMain().Accelerate().Position();
	...
	for (size_t r=0; r<sel.Count(); r++) {
		const Sci::Position rBase = sel.Range(r).Move().Position();
